...
class UsersController{
    ...
    async loginUserAdmin(request, response) {
      ...
      let administrador = 'N';
        ...
        if (usuario.typeUser !== "administrador") {
          throw new Error("Não foi possível realizar o login. Usuário não tem o perfil de administrador.")
        } else {
            administrador = 'S'
        }
        ...
        return response.status(200).send({ token })
            ...
            if (error.message === "Não foi possível realizar o login. Usuário não tem o perfil de administrador.") {
    ...
    return response.status(201).send({ "dados": newUser, "menssage": "Criou um usuário" })
          ...
          if (error.name === 'SequelizeUniqueConstraintError') {
            ...
            if (existeCPFErro || existeEmailErro) {
               return response.status(409).json({ msg: "Erro ao criar usuário", cause: error.message })
            }
          }
          if (error.message.split('\n').length > 1) {
            return response.status(status).json({ msg: "Erro ao criar usuário", causes: error.message.split('\n') })
          }
          console.log(error.message.split('\n'))
          return response.status(status).json({ msg: "Erro ao criar usuário", cause: error.message })
        }
